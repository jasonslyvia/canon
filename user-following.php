...
获取当前 被 浏览的用户信息
======================================*/
$uid = preg_replace('/^.*?\/(\d+)\/following\/?$/', '$1', $_SERVER['REQUEST_URI']);
...
list($query, $post_count, $follow_record) = get_user_following_image($uid, 1, false);
get_header();
...
<div id="luka">
  ...
  <p><a href="/"> <?php echo $name .' 关注的所有人 - '. get_bloginfo(); ?> </a> </p>
</div>
  ...
  <div id="images">
    ...
    <div class="tile" id="userInfo">
      ...
      <div class="plain-stat">
        <p>共关注 <?php echo $post_count;?> 位用户</p>
        <ul class="followees">
          <?php $follow_arr = array_slice($follow_record, 0, 10); ?>
          <?php foreach ($follow_arr as $el) { ?>
            <li><a href="/profile/<?php echo $el['followee_id']; ?>"><?php
